...
El diálogo ConfirmDeletionDialog recibe props title y description personalizadas para inversiones (investments.deleteTitle y investments.deleteConfirmation), mostrando un texto contextual que indica que se eliminará todo el historial asociado. Esto unifica la experiencia con el resto de módulos (Categories, Transactions).
...
- Menú desplegable siempre visible con opción de eliminar (icono Trash2, texto destructivo).

### Menú contextual siempre visible (v0.0.3)

El botón de tres puntos (MoreHorizontal) eliminó las clases opacity-0 group-hover:opacity-100 y ahora es siempre visible en todas las vistas. Esto estandariza la navegación con Categories y Transactions, garantizando accesibilidad en dispositivos táctiles y tablets.
