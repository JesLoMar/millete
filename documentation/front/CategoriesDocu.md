...
- Menú desplegable (DropdownMenu) siempre visible. Opciones: Editar y Eliminar con iconos Edit2 y Trash2.

### Menú contextual (v0.0.3)

El botón de tres puntos (MoreHorizontal) es ahora siempre visible en todas las vistas. Se eliminó la dependencia del hover (opacity-0 group-hover:opacity-100) para garantizar que el menú sea accesible en dispositivos táctiles y tablets, estandarizando la navegación con el resto de módulos.
...
Diálogo de confirmación para eliminar una categoría. Componente reutilizable usado también por transacciones recurrentes e inversiones.
...
- title?: título personalizado (nuevo en v0.0.3). Si no se proporciona, usa categories.deleteTitle.
- description?: descripción personalizada (nuevo en v0.0.3). Si no se proporciona, usa categories.deleteConfirmation con el nombre del elemento.

### Personalización por contexto (v0.0.3)

El diálogo acepta props opcionales title y description que permiten adaptar el texto según el tipo de entidad que se está eliminando. Al eliminar una categoría se usan los textos por defecto. Al eliminar una transacción se pasa title="transactions.deleteTitle" y description="transactions.deleteConfirmation". Al eliminar una inversión se pasa title="investments.deleteTitle" y description="investments.deleteConfirmation". Esto evita tener múltiples diálogos de confirmación duplicados y mantiene la consistencia visual.

### Estructura

- Icono AlertTriangle centrado con fondo destructivo semitransparente.
- Título centrado configurable.
- Descripción centrada configurable.
- Botones: Cancelar (outline) y Eliminar (destructive). Si isDeleting, el botón muestra "Eliminando..." usando la clave i18n common.deleting.
...
1. Usuario hace clic en el menú de tres puntos y selecciona "Editar".
...
1. Usuario hace clic en el menú de tres puntos y selecciona "Eliminar".
2. ConfirmDeletionDialog se abre mostrando el nombre de la categoría con el texto contextual correspondiente.
3. Usuario confirma.
4. deleteCategory.mutateAsync hace DELETE a /categories/:id.
5. Backend desactiva la categoría y desvincula sus transacciones.
6. onSuccess invalida las 5 queries y cierra el diálogo.
