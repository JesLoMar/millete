...
- TransactionItem: id, description, category, categoryColor, amount, date, type ("INCOME" | "EXPENSE"), icon (opcional).
...
### TransactionItem con categoryColor (v0.0.3)

El tipo TransactionItem incluye ahora el campo opcional categoryColor (string) para recibir el color real de la categoría desde el backend. Esto permite que RecentTransactions muestre cada transacción con el color personalizado de su categoría.
...
Usa CATEGORY_ICONS de constants.ts para el icono según el nombre de la categoría. Si la categoría no coincide con ninguna clave, usa ShoppingCart como fallback. El color del badge ahora se obtiene del campo categoryColor recibido del backend.

### Badge de categoría con color real (v0.0.3)

El badge de cada transacción aplica el color real de la categoría mediante estilos inline usando categoryColor. Si la transacción tiene categoryColor, se aplica como color de texto y como fondo con opacidad reducida. Si no tiene categoría o el color no está disponible, se usa el color por defecto (text-muted-foreground bg-muted/10).
...
- Badge de categoría con color real en RecentTransactions: el componente ahora usa el campo categoryColor recibido del backend (incluido en RecentTransactionResponseDTO) para aplicar el color real de cada categoría mediante estilos inline. Si la transacción tiene categoryColor, se aplica como color de texto y como fondo con opacidad. Si no tiene categoría o el color no está disponible, se usa el estilo por defecto. Esto reemplaza el uso del objeto estático CATEGORY_COLORS para las transacciones recientes del dashboard.