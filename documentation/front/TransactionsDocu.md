...
- constants.ts — Constantes compartidas (filtros, frecuencias)
...
### Eliminación de CATEGORY_COLORS (v0.0.3)

Se eliminó el objeto CATEGORY_COLORS que contenía un mapeo fijo de nombres de categorías a clases Tailwind (Alimentación, Hogar, Transporte, Suministros, Ocio). El color de cada categoría ahora se obtiene dinámicamente del backend mediante el campo categoryColor incluido en TransactionResponseDTO, permitiendo que cada usuario vea sus categorías con los colores personalizados que definió.
...
### Interfaz Transaction (v0.0.3)

La interfaz local Transaction incluye ahora el campo opcional categoryColor (string | null) recibido del backend. Este campo contiene el color hexadecimal definido por el usuario para la categoría, permitiendo mostrar badges con colores personalizados.
...
- Descripción (truncada), fecha formateada con formatDate y categoría en badge.
- Si la transacción tiene categoría, el badge usa el color real de la categoría mediante style con color y backgroundColor basado en categoryColor.
- Si la transacción no tiene categoría (huérfana), muestra un icono HelpCircle en color ámbar con el texto "Sin categoría".
- Importe formateado con signo +/- y 2 decimales con separadores españoles.
- Menú desplegable siempre visible con opciones de editar y eliminar.

### Badge de categoría con color real (v0.0.3)

El badge de categoría abandonó el uso del objeto estático CATEGORY_COLORS. Ahora aplica el color real de la categoría mediante estilos inline: el texto usa categoryColor y el fondo usa categoryColor con opacidad reducida. Si la transacción es huérfana (category es null, undefined o "Sin categoría"), se muestra un icono HelpCircle ámbar con el texto "Sin categoría".

### Menú contextual siempre visible (v0.0.3)

El botón de tres puntos (MoreHorizontal) es ahora siempre visible en todas las vistas (desktop y móvil). Se eliminó la dependencia del hover (opacity-0 group-hover:opacity-100) para garantizar accesibilidad en dispositivos táctiles y tablets, estandarizando la navegación con el resto de módulos.
...
Usa ConfirmDeletionDialog (reutilizado desde Categories) con props title y description personalizadas para transacciones. Al confirmar hace DELETE /transactions/:id e invalida las queries de transactions, transactionMetrics y dashboardMetrics. Maneja estado isDeleting para deshabilitar botones durante la petición.

### Modal de confirmación al eliminar (v0.0.3)

Se reemplazó window.confirm por ConfirmDeletionDialog, el mismo componente usado en Categories e Investments. Se pasan las props title y description con claves i18n específicas para transacciones (transactions.deleteTitle y transactions.deleteConfirmation), mostrando un diálogo consistente con el resto de la aplicación.
...
Componente independiente para cada fila de la lista de transacciones (usado por TransactionList y potencialmente otros componentes).

### Props

- transaction: objeto con id, description, category, categoryColor, categoryId, amount, date, type.
- onEdit: callback que recibe la transacción.
- onDelete: callback que recibe la transacción.

### Interfaz Transaction (v0.0.3)

La interfaz incluye el campo opcional categoryColor (string | null) para recibir el color real de la categoría desde el backend.

### Estructura

- Icono circular con flecha según tipo (ArrowUpRight verde para ingreso, ArrowDownLeft rosa para gasto).
- Descripción, fecha formateada y badge de categoría.
- Si la transacción tiene categoría, el badge usa el color real mediante style con categoryColor.
- Si la transacción es huérfana (!tx.category || "Sin categoría"), muestra icono HelpCircle ámbar con el texto "Sin categoría".
- Importe con signo +/- y 2 decimales en formato español.
- Menú desplegable siempre visible con opciones de editar y eliminar (usando claves i18n).

### Badge con color real (v0.0.3)

Se eliminó la dependencia del objeto CATEGORY_COLORS. El badge aplica categoryColor como color de texto y categoryColor con opacidad como fondo mediante estilos inline. Si no hay categoryColor, se usa el estilo por defecto sin color personalizado.

### Menú siempre visible (v0.0.3)

El botón de tres puntos eliminó las clases opacity-0 group-hover:opacity-100 y ahora es siempre visible, igual que en TransactionList, CategoryRow, RecurringTransactionRow y AssetRow.
...
Usa ConfirmDeletionDialog (reutilizado desde Categories) con props title y description personalizadas para transacciones recurrentes. Al confirmar hace DELETE /planned-transactions/:id e invalida ['plannedTransactions'].

### Modal de confirmación al eliminar (v0.0.3)

Se añadieron las props title y description con claves i18n específicas para transacciones recurrentes (transactions.recurring.deleteTitle y transactions.recurring.deleteConfirmation), mostrando un texto contextual que indica que las ejecuciones futuras no se realizarán.
...
El botón de tres puntos eliminó las clases opacity-0 group-hover:opacity-100 y ahora es siempre visible, estandarizando la navegación con el resto de módulos.
...
- Badge de categoría con color real: se eliminó el objeto estático CATEGORY_COLORS de constants.ts. TransactionList y TransactionRow ahora usan el campo categoryColor recibido del backend para aplicar el color real de cada categoría mediante estilos inline (color para el texto, categoryColor con opacidad para el fondo). Si la transacción es huérfana (sin categoría), se muestra un icono HelpCircle en color ámbar con el texto "Sin categoría".
...
- Nuevas claves i18n: se añadieron transactions.noCategory, transactions.deleteTitle, transactions.deleteConfirmation, transactions.recurring.deleteTitle, transactions.recurring.deleteConfirmation, common.delete y common.deleting en los 6 idiomas.