...
- Usa useForm con zodResolver(authFormFieldsSchema).
...
1. Lee token y usuario de secureStorage (métodos getToken() y getUser()).
2. Si hay token:
...
- secureStorage: utilidad compartida que abstrae el almacenamiento seguro.
  ...
  - secureStorage.getUser() / secureStorage.setUser(user) para el perfil.
