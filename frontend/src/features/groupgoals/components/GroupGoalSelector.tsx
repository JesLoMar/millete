      ...
      <div className="max-w-2xl mx-auto space-y-4 sm:space-y-6 pt-6 sm:pt-12 px-4 sm:px-0">
        ...
        <div className="space-y-3 sm:space-y-4">
          {Array.from({ length: 5 }).map((_, i) => (
            <div
              key={`skeleton-${i}`}
              className="h-20 sm:h-24 bg-muted animate-pulse rounded-xl"
            />
          ))}
        </div>
      </div>
        ...
        <div className="space-y-3 sm:space-y-4">
          {goals.map((goal) => (
            <Card
              ...
              aria-label={t('groupGoals:selectFamily', {
                name: goal.name,
              })}
            >
                ...
                <div className="space-y-1.5 min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {goal.isAdmin && (
                      <Badge
                        variant="outline"
                        className="border-warning/30 text-warning gap-1 text-xs shrink-0"
                      >
                        <Crown
                          className="size-3"
                          aria-hidden="true"
                  ...
                  <p className="text-xs sm:text-sm text-muted-foreground truncate">
                    <Users
                      className="size-3 sm:size-3.5 inline mr-1"
                      aria-hidden="true"
                    />
