...
export function ProgressBar({ contributions, monthlyGoal }: ProgressBarProps) {
  ...
  return (
    <div className="space-y-1">
      <div 
        className="h-3 w-full bg-secondary rounded-full overflow-hidden flex"
        role="progressbar"
        aria-valuenow={Math.round(totalPercentage)}
        aria-valuemin={0}
        aria-valuemax={100}
        aria-label={t("family.progressBar.label", "Progreso del objetivo mensual")}
      >
      ...
      <p className="text-xs text-muted-foreground">
        {totalContributed.toLocaleString()} € {t("family.progressBar.of", "de")} {monthlyGoal.toLocaleString()} €
        {" · "}
