...
const RECURRING_TRANSACTION_QUERY_KEYS = [
  'plannedTransactions',
] as const
...
function invalidateTransactions(queryClient: QueryClient) {
  return invalidateQueries(queryClient, TRANSACTION_QUERY_KEYS)
}

function invalidateRecurringTransactions(queryClient: QueryClient) {
  return invalidateQueries(
    queryClient,
    RECURRING_TRANSACTION_QUERY_KEYS,
  )
}
...
export const useTransactionMutations = () => {
  ...
  const createRecurring = useMutation({
    ...
    onSuccess: async () => {
      await invalidateRecurringTransactions(queryClient)

      notify.success(
        t('alerts.createRecurringSuccess'),
      )
    },
  ...
  const updateRecurring = useMutation({
    ...
    onSuccess: async () => {
      ...
      notify.success(
        t('alerts.updateRecurringSuccess'),
      )
    },
  ...
  const deleteRecurring = useMutation({
    ...
    onSuccess: async () => {
      ...
      notify.success(
        t('alerts.deleteRecurringSuccess'),
      )
    },
