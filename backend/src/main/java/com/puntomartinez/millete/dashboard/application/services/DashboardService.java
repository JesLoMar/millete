...
public class DashboardService implements GetDashboardDataUseCase {
    ...
    public DashboardMetricsResponseDTO getMetrics(UUID userId, String period) {
        ...
        List<Transaction> currentTransactions = transactionRepository.findByUserIdAndDateBetween(userId, currentRange[0], currentRange[1]);

        List<Transaction> previousTransactions = transactionRepository.findByUserIdAndDateBetween(userId, previousRange[0], previousRange[1]);
    ...
    public DashboardCategoriesResponseDTO getCategories(UUID userId, String period) {
        ...
        List<Transaction> expenses = transactionRepository.findByUserIdAndDateBetween(userId, range[0], range[1]).stream().filter(t -> t.getType() == TransactionType.EXPENSE).toList();

        BigDecimal totalExpenses = expenses.stream().map(t -> t.getAmount().abs()).reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        if (!orphanTransactions.isEmpty()) {
            ...
            if (existingOrphan.isPresent()) {
                ...
                categoryItems.add(new CategoryExpenseItemResponseDTO("Sin categoría", combinedAmount, combinedPercentage, combinedCount));
            } else {
                categoryItems.add(new CategoryExpenseItemResponseDTO("Sin categoría", orphanAmount, orphanPercentage, orphanTransactions.size()));
            }
        }
    ...
    public DashboardBudgetsResponseDTO getBudgets(UUID userId, String period) {
        ...
        List<Transaction> periodTransactions = transactionRepository.findByUserIdAndDateBetween(userId, range[0], range[1]);
    ...
    public DashboardTransactionsResponseDTO getRecentTransactions(UUID userId, int limit) {
        ...
        List<RecentTransactionResponseDTO> transactionDTOs = recentTransactions.stream().map(t -> {
            ...
            return new RecentTransactionResponseDTO(t.getId(), t.getDescription(), catName, catColor, t.getCategoryId(), t.getAmount(), t.getDate(), t.getType().name());
        }).collect(Collectors.toList());
...
// =====================================================
    ...
    public InvestmentMetricsResponseDTO getInvestmentMetrics(UUID userId, String period) {
        ...
        double returnTrend = investedCapital.compareTo(BigDecimal.ZERO) > 0 ? currentReturn.divide(investedCapital, 4, RoundingMode.HALF_UP).multiply(new BigDecimal("100")).doubleValue() : 0.0;
    ...
    public InvestmentDistributionResponseDTO getInvestmentDistribution(UUID userId, String period) {
        ...
        Map<String, List<Investment>> byType = investments.stream().collect(Collectors.groupingBy(i -> i.getType().name()));
        ...
        for (Map.Entry<String, List<Investment>> entry : byType.entrySet()) {
            ...
            distribution.add(new InvestmentDistributionItemDTO(entry.getKey(), typeValue, calculatePercentage(typeValue, totalValue), CHART_COLORS[colorIndex % CHART_COLORS.length]));
            colorIndex++;
        }
    ...
    private LocalDateTime[] getDateRange(String period) {
        ...
        return switch (period.toLowerCase()) {
            case "week" ->
                    new LocalDateTime[]{now.with(DayOfWeek.MONDAY).withHour(0).withMinute(0).withSecond(0), now.with(DayOfWeek.SUNDAY).withHour(23).withMinute(59).withSecond(59)};
            case "month" ->
                    new LocalDateTime[]{now.withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0), now.with(TemporalAdjusters.lastDayOfMonth()).withHour(23).withMinute(59).withSecond(59)};
            case "year" ->
                    new LocalDateTime[]{now.withDayOfYear(1).withHour(0).withMinute(0).withSecond(0), now.with(TemporalAdjusters.lastDayOfYear()).withHour(23).withMinute(59).withSecond(59)};
            default -> throw new IllegalArgumentException("Invalid period: " + period);
        };
    }
    ...
    private double calculateTrend(BigDecimal current, BigDecimal previous) {
        ...
        return current.subtract(previous).multiply(new BigDecimal("100")).divide(previous.abs(), 1, RoundingMode.HALF_UP).doubleValue();
    }

    private double calculatePercentage(BigDecimal part, BigDecimal total) {
        if (total.compareTo(BigDecimal.ZERO) == 0) return 0.0;
        return part.multiply(new BigDecimal("100")).divide(total, 1, RoundingMode.HALF_UP).doubleValue();
    }

    private int comparePriority(String a, String b) {
        Map<String, Integer> priorityOrder = Map.of("HIGH", 3, "MEDIUM", 2, "LOW", 1);
        return Integer.compare(priorityOrder.getOrDefault(a, 0), priorityOrder.getOrDefault(b, 0));
    }
    ...
    private DashboardHistoryResponseDTO getMonthlyHistory(UUID userId) {
        ...
        while (weekStart.isBefore(today.plusDays(1)) && weekNumber <= 5) {
            ...
            BigDecimal weekExpenses = transactionRepository.findByUserIdAndDateBetween(userId, startDateTime, endDateTime).stream().filter(t -> t.getType() == TransactionType.EXPENSE).map(t -> t.getAmount().abs()).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private DashboardHistoryResponseDTO getYearlyHistory(UUID userId) {
        ...
        for (int month = 1; month <= 12; month++) {
            ...
            BigDecimal monthExpenses = transactionRepository.findByUserIdAndDateBetween(userId, startDateTime, endDateTime).stream().filter(t -> t.getType() == TransactionType.EXPENSE).map(t -> t.getAmount().abs()).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private List<CategoryExpenseItemResponseDTO> groupSmallCategories(List<CategoryExpenseItemResponseDTO> categories, BigDecimal totalExpenses) {
        ...
        if (othersAmount.compareTo(BigDecimal.ZERO) > 0) {
            ...
            mainCategories.add(new CategoryExpenseItemResponseDTO("Otros", othersAmount, othersPercentage, othersCount));
        }
