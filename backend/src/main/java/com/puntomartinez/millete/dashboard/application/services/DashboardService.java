...
public class DashboardService implements GetDashboardDataUseCase {
    ...
    public DashboardMetricsResponseDTO getMetrics(
        ...
        BigDecimal currentIncome =
                sumByType(currentTransactions, TransactionType.INCOME);

        BigDecimal currentExpenses =
                sumByType(currentTransactions, TransactionType.EXPENSE);

        BigDecimal currentBalance =
                currentIncome.subtract(currentExpenses);

        BigDecimal previousIncome =
                sumByType(previousTransactions, TransactionType.INCOME);

        BigDecimal previousExpenses =
                sumByType(previousTransactions, TransactionType.EXPENSE);
    ...
    public DashboardCategoriesResponseDTO getCategories(
        ...
        for (Map.Entry<UUID, List<Transaction>> entry :
                byCategory.entrySet()) {
        ...
        if (!orphanTransactions.isEmpty()) {
            BigDecimal orphanAmount =
                    orphanTransactions.stream()
                            .map(t -> t.getAmount().abs())
                            .reduce(BigDecimal.ZERO, BigDecimal::add);
            ...
            Optional<CategoryExpenseItemResponseDTO> existingOrphan =
                    categoryItems.stream()
                            .filter(item ->
                                    "Sin categoría".equals(item.name()))
                            .findFirst();

            if (existingOrphan.isPresent()) {
                CategoryExpenseItemResponseDTO old =
                        existingOrphan.get();

                BigDecimal combinedAmount =
                        old.amount().add(orphanAmount);
    ...
    public DashboardBudgetsResponseDTO getBudgets(
        ...
        List<Category> categoriesWithBudget =
                categoryRepository.findCategoriesWithBudgetByUserId(
                        userId
                );
    ...
    public InvestmentMetricsResponseDTO getInvestmentMetrics(
        ...
        for (Investment inv : currentInvestments) {
            currentValue =
                    currentValue.add(inv.getCurrentValue());

            currentReturn =
                    currentReturn.add(inv.getProfitOrLoss());

            investedCapital =
                    investedCapital.add(inv.getInvestedCapital());
        }

        double returnTrend =
                investedCapital.compareTo(BigDecimal.ZERO) > 0
                        ? currentReturn
                                .divide(
                                        investedCapital,
                                        4,
    ...
    public InvestmentEvolutionResponseDTO getInvestmentEvolution(
        ...
        return switch (period.toLowerCase()) {
            case "week" ->
                    getWeeklyInvestmentEvolution(userId);
            case "month" ->
                    getMonthlyInvestmentEvolution(userId);
            case "year" ->
                    getYearlyInvestmentEvolution(userId);
            default ->
    ...
    private BigDecimal getPortfolioValueAtDate(
        ...
        for (Investment inv : investments) {
            if (inv.getPurchaseDate() != null
                    && inv.getPurchaseDate().isBefore(date)
                    && inv.isActive()) {
    ...
    public InvestmentDistributionResponseDTO getInvestmentDistribution(
        ...
        Map<String, List<Investment>> byType =
                investments.stream()
                        .collect(
        ...
        for (Map.Entry<String, List<Investment>> entry :
                byType.entrySet()) {
